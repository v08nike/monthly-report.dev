...
function init_all_event_handler(root, map_info) {
   ...
   */
  root.querySelector('[tagId="Search"]').addEventListener(
    'click',
    function (root, event) {
      //display search modal:
      root.querySelector('[tagId="search-modal"]').style.display = 'block'
      // set all event for search modal:
      init_search_modal_handler(root, map_info)
    }.bind(this, root),
  )
  // init sort-button event listner
  root.querySelector('[tagId="Sort"]').addEventListener(
    'click',
    function (root, event) {
      //display sort modal:
      root.querySelector('[tagId="sort-modal"]').style.display = 'block'
      // set all event for sort modal:
      init_sort_modal_handler(root, map_info)
    }.bind(this, root),
  )
  // init filter-button event listner
  root.querySelector('[tagId="Filter"]').addEventListener(
    'click',
    function (root, event) {
      //display filter modal:
      root.querySelector('[tagId="filter-modal"]').style.display = 'block'
      // set all event for filter modal:
      init_filter_modal_handler(root, map_info)
    }.bind(this, root),
  )
  // init summary-button event listner
  root.querySelector('[tagId="Summary"]').addEventListener(
    'click',
    function (root, event) {
  ...
  root.querySelector('[tagId="Refresh"]').addEventListener(
    'click',
    function (event) {
      //init all filters:
  ...
  root.querySelectorAll('.close').forEach((item) => {
    item.addEventListener('click', function (event) {
      root.querySelector('[tagId="search-modal"]').style.display = 'none'
      root.querySelector('[tagId="sort-modal"]').style.display = 'none'
      root.querySelector('[tagId="filter-modal"]').style.display = 'none'
      root.querySelector('[tagId="summary-modal"]').style.display = 'none'
    })
  })
...
function init_search_modal_handler(root, map_info) {
  root.querySelector('[tagId="hmvi_explorer_input_text"]').focus()
  root
    .querySelector('[tagId="hmvi_explorer_input_text"]')
    .addEventListener(
      'input',
      search_input_event_listner.bind(this, root, map_info),
    )
  root.querySelector('[tagId="input_text_reset"]').addEventListener(
    'click',
    function (root, e) {
      e.preventDefault()
...
function removeMapFilterActiveClass(root) {
  root.querySelector('[tagId="market_performance"]').classList.remove('active')
  root.querySelector('[tagId="situation"]').classList.remove('active')
  root.querySelector('[tagId="outlook"]').classList.remove('active')
}

//set all sort modal event handler
function init_sort_modal_handler(root, map_info) {
  root.querySelector('[tagId="market_performance"]').addEventListener(
    'click',
    function (root, map_info, e) {
      e.preventDefault()
      removeMapFilterActiveClass(root)
      root.querySelector('[id="map-legend-title"]').innerHTML = 'Market Rank'
      root.querySelector('[tagId="market_performance"]').classList.add('active')
      update_map_and_chart(root, map_info)
    }.bind(this, root, map_info),
  )
  root.querySelector('[tagId="situation"]').addEventListener(
    'click',
    function (root, map_info, e) {
      e.preventDefault()
      removeMapFilterActiveClass(root)
      root.querySelector('[id="map-legend-title"]').innerHTML = 'Situation'
      root.querySelector('[tagId="situation"]').classList.add('active')
      update_map_and_chart(root, map_info)
    }.bind(null, root, map_info),
  )
  root.querySelector('[tagId="outlook"]').addEventListener(
    'click',
    function (root, map_info, e) {
      e.preventDefault()
      removeMapFilterActiveClass(root)
      root.querySelector('[id="map-legend-title"]').innerHTML = 'Outlook'
      root.querySelector('[tagId="outlook"]').classList.add('active')
      update_map_and_chart(root, map_info)
    }.bind(this, root, map_info),
  )
}
...
function init_filter_modal_handler(root, map_info) {
  root.querySelector('[tagId="market-rank-range"]').addEventListener(
    'mouseup',
    function (root, map_info, e) {
      e.preventDefault()
      update_map_and_chart(root, map_info)
...
function calc_and_display_summary(root, map_info) {
  ...
  root.querySelector(
    "[tagId='underperforming-outperform']",
  ).innerHTML = stringOfvalueAndpercend(
    underperform_outperform,
    init_map_length,
  )
  root.querySelector(
    "[tagId='underperforming-track']",
  ).innerHTML = stringOfvalueAndpercend(underperform_track, init_map_length)
  root.querySelector(
    "[tagId='underperforming-underperform']",
  ).innerHTML = stringOfvalueAndpercend(
    underperform_underperform,
    init_map_length,
  )
  root.querySelector(
    "[tagId='underperforming-total']",
  ).innerHTML = stringOfvalueAndpercend(
  ...
  root.querySelector("[tagId='summary-content']").innerHTML =
    'Displaying ' +
    total_total_count +
    ' of ' +
    init_map_length +
  ...
  root.querySelector('[tagId="summary-modal"]').style.display = 'block'
}

function refresh(root, map_info) {
  selected_places = []
  root.querySelector('[tagId="slider-min"]').value = 0
  root.querySelector('[tagId="slider-max"').value = 100
  root.querySelector('.slider-x').value = 100
  root.querySelector('.slider-n').value = 0
  root.querySelector("[tagId='underperform']").value = 'on'
  root.querySelector("[tagId='track']").value = 'on'
  root.querySelector("[tagId='outperform']").value = 'on'
  root.querySelector("[tagId='outperforming']").value = 'on'
  root.querySelector("[tagId='underperforming']").value = 'on'
  root.querySelector("[tagId='tracking']").value = 'on'
  // update the checked value
  root.querySelector("[tagId='underperform']").checked = true
  root.querySelector("[tagId='track']").checked = true
  root.querySelector("[tagId='outperform']").checked = true
  root.querySelector("[tagId='outperforming']").checked = true
  root.querySelector("[tagId='underperforming']").checked = true
  root.querySelector("[tagId='tracking']").checked = true
  // sort by market rank
  root.querySelector('[tagId="hmvi_explorer_input_text"').value = ''
  root.querySelector('[tagId="hmvi_explorer_itme_selected"').innerHTML = ''
...
function get_matched_items(root, search_value, data) {
  let matched_items = []
  let map_data_type = root.getAttribute('dataName')

  data.forEach((element) => {
    if (
      element['place_name'].toLowerCase().includes(search_value.toLowerCase())
    ) {
...
function update_data_by_filter(root, map_data, sort_type) {
  ...
  map_data_type = root.getAttribute('dataName')
  ...
  range_max = root.querySelector("[tagId='slider-max']").value
  updated_data = updated_data.filter((item) => {
    if (item["market_performance_pctl"] <= range_max && item["market_performance_pctl"] >= range_min) {
      return true
    } else {
